...
safety_outcome <- c("dis-15-157", "dis-15-1516", "dis-15-2495", "trait-7-1")
####
...
res_pan <- merge(res_pan, dt_gene_region[,.(id, study, hgnc)], by.x = "id.exposure", by.y = "id")
res_pan <- merge(res_pan, df_index[,.(id, clean_variable_name)], by.x = "id.outcome", by.y = "id")
...
dt_title <- data.table(title = paste0("ST", c(1:4)),
                       caption = c( "Description of the datasets used as outcomes for Two-Sample Mendelian randomization.",

...
res_multicis <- merge(res_multicis, dt_gene_region, by.x = "id.exposure", by.y = "id")
res_multicis <- merge(res_multicis, df_index[,.(id, clean_variable_name)], by.x = "id.outcome", by.y = "id")
...
from_resunicis_to_datballoon <- function(dat) {
  ...
  dat_balloon[,log10_pval:=-log10(pval)] # expression(-log[10](p))
...
dat_balloon <- from_resunicis_to_datballoon(res_unicis[id.outcome%in%c(efficacy_outcome, safety_outcome) &
                                                         study == "deCODE",])
balloon_plot <- plot_balloon2(dat_balloon, subdivide_by_category = TRUE, name_pval = "-log10(p)", name_zcore = "Beta")
...
volcano <- res_unicis[study=="deCODE" & outcome %in% c("venous_thromboembolism"), ][order(-abs(b))]
volcano[,logpval:=-log10(pval)]
ntest<-volcano$hgnc%>%unique%>%length
volcano[, diffexpressed := "Neutral"]
volcano[, diffexpressed := diffexpressed %>% ifelse(b > 0 & pval < 0.05/ntest, "Higher risk", .) %>%
          ifelse(b < 0 & pval < 0.05/ntest,  "Lower risk", .)]
volcano[, diffexpressed := factor(diffexpressed, levels = c("Higher risk","Lower risk", "Neutral"))]
volcano[pval < 0.05/ntest & steiger_dir == TRUE & fstat > 10, delabel := hgnc]
volcano[is.na(delabel), diffexpressed := "Neutral"]
volcano<-volcano[outcome=="venous_thromboembolism",]

lim_y <- max(volcano$logpval)
volcanoplot <-  ggplot(data = volcano, aes(x = exp(b), y = logpval, col = diffexpressed,  label=delabel)) +
  geom_point(size = 1, alpha = ifelse(volcano[, is.na(delabel)], 0.5, 1)) +
  theme_bw() +
  scale_color_manual(values=c("#9E131E", "#5884E5", "#B0B0B0")) +
  geom_hline(yintercept = -log10(0.05/ntest), color = "grey60", linewidth = 1) +
  ggrepel::geom_text_repel() +
  scale_y_continuous(name = expression(-log[10](Pval))) +
  scale_x_continuous(name = "OR of VTE",trans='log10') +
  theme(    panel.grid.major.y = element_blank(),
            ...
            # legend.position = c(0.7,80),
            # legend.title = element_text("Association"),
 ...
 guides(color=guide_legend(title="Association")) +
  theme(legend.position = c(0.2, 0.8))


#######all cohorts PPH4#######
data <- res_unicis[id.outcome%in%c(efficacy_outcome, safety_outcome) &
                     study %in% c("deCODE", "FENLAND", "ARIC") ,]
data <- data[clean_variable_name=="Venous thromboembolism",]
pph4_plot <- ggplot(data, aes(x = reorder(hgnc, -posprob_coloc_PPH4), y = posprob_coloc_PPH4)) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.5, colour = "gray25")  +
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  scale_fill_grey() +
  labs(x = element_blank(), y = "PPH4") +
...
###### cowplot#####
left_side <- plot_grid(  volcanoplot, pph4_plot, labels = c('A)', 'C)'), label_size = 12, ncol = 1)
threeplot_panel <- plot_grid(left_side, balloon_plot, labels = c('', 'B)'), label_size = 12, nrow = 1, rel_widths = c(0.8, 1))
threeplot_panel
ggsave(threeplot_panel, filename = paste0("Results/Figure_panel2.png"),
       width = 813/72,height = 674/72,units="in",scale=1, device = "png")

#####All cohorts balloon plot plot only PPH4


#####scatter plot pan#####
res_pan[method %in% c("Inverse variance weighted","Weighted median", "Contamination mixture", "MR-PRESSO (outlier-corrected)"), causal_multicis:= all(pval<0.05) & (all(b<0)|all(b>0)) , by = c("id.exposure", "id.outcome")]
harm_univariate_pan <- fread( "Data/Modified/harm_univariate_pan.txt")
...
list_pan <- map(list(res_pan, harm_univariate_pan), cleanify)
outcome_to_include <- c("Cardioembolic stroke", "Venous thromboembolism", "Bleeding")
...
ggsave(filename = paste0("Results/F2F11scatter_plot_pan.png"),
       width = 700/72,height = 700/72,units="in",scale=1, device = "png")

#####scatter plot multi cis#####
res_multicis<-res_multicis[method %in% c("Inverse variance weighted","Weighted median", "Contamination mixture", "MR-PRESSO (outlier-corrected)"), causal_multicis:= all(pval<0.05) & (all(b<0)|all(b>0)) , by = c("id.exposure", "id.outcome")]
harm_univariate <- fread( "Data/Modified/harm_univariate.txt")
res_multicis[, label := paste0(hgnc, "_", study)]
harm_univarite <- cleanify(harm_univariate)
outcome_to_include <- c("Cardioembolic stroke", "Venous thromboembolism", "Bleeding")

k2 <-res_multicis[hgnc%in%c("F11", "F2")&
                     clean_variable_name %in% outcome_to_include & 
                     study %in% c("deCODE", "FENLAND") & exposure%in%c("2190_55", "5316_54", "fenland2190_55", "fenland5316_54")]
k2 <- k2[method%in%c("MR-PRESSO (outlier-corrected)", "Contamination mixture",
                     "Inverse variance weighted", "Weighted median")]
...
ggsave(filename = paste0("Results/F2F11scatter_plot_multicis.png"),
       width = 700/72,height = 700/72,units="in",scale=1, device = "png")
...
multi_forest <- res_multicis[hgnc %in% c("F2", "F11") & method == "Inverse variance weighted" &
...
multi_forest <- multi_forest[, .(Category, clean_variable_name, hgnc, study ,exposure, b, se, pval, lci, uci,
                                 nsnp), ][order(Category, clean_variable_name, hgnc, study ,exposure)]
...
# multi_forest[,uci:=b+1.96*se]
# 
# multi_forest <- multi_forest[method == "Inverse variance weighted", 
#                              .(Category, clean_variable_name, hgnc, study ,exposure, b, pval, lci, uci,
#                                  nsnp), ][order(Category, clean_variable_name, hgnc, study ,exposure)]
# 
# 
...
# 
# my_forest_fancy(
#   data= multi_forest[study=="ARIC"],
#   col.right = c("pval"),
#   col.right.heading = list(c("Effect (95% CI)", "P value")),
#   exponentiate = TRUE,
#   xlab="",
#   col.left = c("exposure", "nsnp"),
...
########Tissue Expression#########
gene_list <- res_unicis$hgnc %>% unique()
...
fwrite(dt_united, "Data/Modified/dt_tissue_specificity.txt")
